...
\begin{frame}{Benotung}
 \begin{itemize}
     \item<1-> Klausur: $40\%$
     \item<2-> 1 Klein-Projektaufgabe: $20\%$
     \only<3>{
	 	\begin{itemize}
...
\begon{frame}{Exkurs: Warum Python?}
    \note{
        \only<1-2>{
		   \begin{itemize}
    ...
    Wichtig in Programmierung: Weniger eine konkrete Sprache, mehr breites Wissen und Verständnis von Konzepten. Also: Richtige Programmiersprache für das Problem wählen! \only<2->{Aber: Was heißt ``richtig?''}\only<4->{\textbf{ Und ist Python ``richtig''?}}
    \only<3->{
        \begin{itemize}
